...
export function buildScript(orderedNodes: Node<MLNodeData>[], options: BuildScriptOptions = {}): string {
  ...
  // 1. Base imports only — node-specific imports are emitted LAZILY inside
  //    each node's section so one broken import can never kill every node.
  const baseImports = ['import pandas as pd', 'import numpy as np', 'import json'];
  ...
  orderedNodes.forEach((n, i) => {
    if (normalizeTitle(n.data.title) === 'Note') return; // notes never execute
    body += `# ===== [${i + 1}/${orderedNodes.length}] ${n.data.title} =====\n`;
    body += `print("__MLPIPE_NODE__::" + json.dumps({"id": ${pyStr(n.id)}, "title": ${pyStr(n.data.title)}}))\n`;
    (NODE_IMPORTS[normalizeTitle(n.data.title)] ?? []).forEach((imp) => { body += imp + '\n'; });
